...
class SignalCNN(nn.Module) :
    ...
    def __init__(self, n_feature1, n_feature2, n_class = 2):
        super().__init__()            # input shape (1, 64, 20)
        self.conv1 = nn.Sequential(
            ...
            nn.Tanh(),
            nn.MaxPool2d(kernel_size=2),    # 在 2x2 空间里向下采样, output shape (16, 32, 10)
        )
        self.conv2 = nn.Sequential(
            ...
            nn.Tanh(),
            nn.MaxPool2d(kernel_size=2),    # 在 2x2 空间里向下采样, output shape (32, 16, 5)
        )
        self.fc1 = nn.Linear(32*16*5, n_class)
...
if __name__ == "__main__":
    ...
    for m in scnn.modules():
        if isinstance(m, (nn.Conv2d, nn.Linear)):
            nn.init.xavier_normal_(m.weight)
    scnn.cuda()

    optimizer = optim.SGD(scnn.parameters(), lr=LR, weight_decay=LR_DECAY)
